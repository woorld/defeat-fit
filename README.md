...
VRChat及びVRChatアバター用対戦ギミックと併用する想定で作られていますが、設定次第でほかの用途にも使えます。
...
- ホーム画面中央左のボタンが緑色の「OSC受信中」になっている
  - 黄色の「OSC受信停止中」になっている場合、ボタンを押して上記の状態にしてください
- VRChatのExpressionMenuでOSCを有効にしている
  - 「オプション」→「OSC」→「有効」でオンにできます
- VRChat側で「DefeatFit にOSCデータを送信中」という通知が表示される
  - DefeatFit側でOSCをオンにしてから1分程度時間を要する場合があります
  - 初回以降は確認不要です

負けた時にホーム画面左側の数字が+1されていれば、設定が正しく行われています。
...
各メニューをクリックすることで、1セットあたりの秒数/回数の調節し、タイマーやカウンターに設定することが可能です。

### 6. 統計への記録

プリセット内のメニューをすべて行ったあと、ホーム画面右上の「全部やった」ボタンを押すことで、今日の記録を統計に記録することができます。
...
ギミックでの対戦中以外に意図せずカウントが行われてしまった場合、ホーム画面左側の-ボタンを押すことで、カウントを-1することができます。

### タイマー・カウンター機能

ホーム画面のメニューをクリックで表示されるダイアログ、もしくは下部メニューからタイマー・カウンター画面にアクセスできます。

秒数制のメニューの場合はタイマー、回数制のメニューの場合はカウンターを使うことで、トレーニングを便利に進められます。
