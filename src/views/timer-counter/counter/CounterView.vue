...
const hasResolvedInitialPending = ref(false);
...
// NOTE: OSCサーバー開始中にUprightのリッスンを開始しようとするのを防ぐ
if (oscStore.oscStatus !== 'PENDING') {
  hasResolvedInitialPending.value = true;
}
else {
  const unwatch = watch(() => oscStore.oscStatus, (newValue) => {
    if (newValue !== 'PENDING') {
      unwatch();
      hasResolvedInitialPending.value = true;
    }
...
<template>
  <VContainer class="d-flex justify-center align-center flex-column ga-8 h-100">
    <template v-if="hasResolvedInitialPending">
      <UprightIndicator
